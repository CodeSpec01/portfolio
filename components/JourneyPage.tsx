...
const TEAM: TeamMember[] = [
  { name: "Alex Dev", role: "Full Stack Arch", image: "https://i.pravatar.cc/150?u=a" },
  { name: "Sam Design", role: "UI/UX Lead", image: "https://i.pravatar.cc/150?u=b" },
];
...
const JourneyPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#030014] text-slate-200 font-sans selection:bg-yellow-400 selection:text-purple-900 overflow-x-hidden">
      ...
      <div className="relative z-10 bg-[#030014]/50 backdrop-blur-sm border-y border-white/5">
         <TimelineWebsite />
      </div>
          ...
          <div className="mb-16 md:flex justify-between items-end">
            <div>
              <h2 className="text-4xl font-bold text-white tracking-tight mb-4">
                The <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-yellow-400">Tech Stack</span>
              </h2>
              <p className="text-slate-400 max-w-md">
                Tools chosen not because they are trendy, but because they provide absolute control.
              </p>
            </div>
            <div className="hidden md:block h-px w-32 bg-gradient-to-r from-transparent via-slate-700 to-transparent mb-4"></div>
          </div>
      ...
      {/* --- TEAM SECTION --- */}
      <section className="relative z-10 py-24 px-6 border-t border-white/5 bg-gradient-to-b from-[#030014] to-purple-950/20">
          ...
          <div className="flex flex-wrap justify-center gap-16">
            {TEAM.map((member) => (
              <div key={member.name} className="group relative">
                {/* Glowing Orbit Ring */}
                <div className="absolute -inset-1 w-36 h-36 rounded-full bg-gradient-to-r from-purple-600 to-yellow-400 opacity-20 group-hover:opacity-100 blur transition duration-500"></div>
      ...
      <footer className="relative z-10 py-16 text-center overflow-hidden">
        {/* Footer Glow */}
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[600px] h-[300px] bg-yellow-400/10 blur-[100px] rounded-full pointer-events-none" />
        ...
        <button className="group relative z-20 px-8 py-3 rounded-full bg-slate-900 border border-slate-800 text-white font-bold hover:border-yellow-400/50 transition-all overflow-hidden flex items-center gap-3 mx-auto">
          <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-yellow-400/10 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000 ease-in-out" />
